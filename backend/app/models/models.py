...
class BudgetLine(Base):
    ...
    notes      = Column(Text, nullable=True)
    sort_order = Column(Integer, default=0)
    # 'manual' = user-added; 'auto' = pulled from the system (a section's "Pull
    # from System", or an income candidate the user ticked); 'constant' = seeded
    # from a BudgetLineTemplate, existence only, never an amount. source_key
    # identifies what an auto line tracks (e.g. "supplier:42", "income:po:POH123")
    # so a pull updates the right line and never duplicates it. Manual lines are
    # never auto-touched.
...
class BudgetLineValue(Base):
    ...
    # Set when a user hand-edits this cell — a later pull then leaves it alone,
    # so manual edits to an auto line survive.
    is_overridden = Column(Boolean, nullable=False, default=False, server_default="false")
