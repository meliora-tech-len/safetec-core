...
class BudgetLine(Base):
    ...
    notes      = Column(Text, nullable=True)
    sort_order = Column(Integer, default=0)
    # 'manual' = user-added; 'auto' = generated by refresh-from-system. source_key
    # identifies what an auto line tracks (e.g. "supplier:42") so a refresh updates
    # the right line and never duplicates it. Manual lines are never auto-touched.
...
class BudgetLineValue(Base):
    ...
    # Set when a user hand-edits this cell — a refresh-from-system then leaves it
    # alone so manual edits to an auto line survive the next pull.
